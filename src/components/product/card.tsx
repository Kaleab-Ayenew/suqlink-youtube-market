...
export default function Card({ video }: { video: Video }) {
  // const { name, slug, image, shop, is_external } = product ?? {};
  const { platform_id, video_price, video_info } = video ?? {};
  ...
  // });
  const { price, basePrice } = usePrice({
    amount: parseFloat(video.video_price),
    baseAmount: parseFloat(video.video_price),
  });
  ...
  const goToDetailsPage = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    Router.push(routes.videoUrl(platform_id));
  };

  const { t } = useTranslation('common');
  const isFreeItem = isFree(video?.n_price ?? video?.n_price);
  return (
    <motion.div variants={fadeInBottomWithScaleX()} title={video_info.title}>
      ...
      <div className="flex items-start justify-between pt-3.5">
        <div className="relative flex h-8 w-8 flex-shrink-0 overflow-hidden 4xl:h-9 4xl:w-9">
          <Image
            ...
            quality={100}
            fill
            src={ch_info?.thumbnail_url ?? placeholder}
        ...
        <div className="-mt-[1px] flex flex-col truncate ltr:mr-auto ltr:pl-2.5 rtl:ml-auto rtl:pr-2.5 rtl:text-right">
          ...
          >
            <AnchorLink href={routes.videoUrl(platform_id)}>
              {video_info.title}
            </AnchorLink>
          </h3>
          <AnchorLink
            href={routes.channelUrl(ch_info?.id)}
