...
import PaymentInfoForm from '../auth/payment-info-form';
...
const ChapaPaymentModal = dynamic(
  () => import('@/components/auth/payment-info-form'),
  { ssr: false }
);

const DeleteCardModal = dynamic(() => import('@/components/card/delete-view'));

function renderModalContent(view: MODAL_VIEWS, platform_id: string | null) {
  switch (view) {
    case 'REGISTER':
    ...
    case 'CHAPA_PAYMENT_MODAL':
      return <ChapaPaymentModal platform_id={platform_id} />;
    default:
      return null;
  }
}
            ...
            <div className="relative z-50 inline-block min-h-screen w-full transform overflow-hidden text-start align-middle transition-all xs:min-h-[auto] xs:w-auto">
              <div className="relative flex min-h-screen items-center overflow-hidden xs:block xs:min-h-[auto] xs:rounded-md">
                ...
                <div className="h-full w-full">
                  {view && renderModalContent(view, data?.platform_id)}
                </div>
              </div>
            </div>
          </Transition.Child>
