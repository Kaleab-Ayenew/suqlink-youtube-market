...
export function useVideo(platform_id: string) {
  const { locale: language } = useRouter();

  // console.log({ slug, language });

  const { data, isLoading, error } = useQuery<Video, Error>(
    [API_ENDPOINTS.VIDEOS, { platform_id }],
    () => client.videos.get({ platform_id })
  );
  return {
    video: data,
    isLoading,
...
export function useClientVideoList(): {
  videos: Video[];
  isLoading: boolean;
  error?: Error | null;
} {
...
export function usePurchasedVideosList(): {
  ...
  const { data, isLoading, error } = useQuery<PurchasedVideo[], Error>(
    [API_ENDPOINTS.PURCHASED_VIDEOS],
    () => client.videos.purchased()
  );
  return {
    videos: data ?? [],
    isLoading,
